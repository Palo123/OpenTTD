...
EngineID ConsistFirstEngine(Train *head, Train *u)
{
	...
	for (Train *t = head; t != nullptr && t != u; t = t->Next()) {
		if (t->IsEngine()) first_engine = t->engine_type;
	}
	for (Train *t = head->Last(); t != nullptr && t != u; t = t->Previous()) {
...
static Train *ReverseTrainChain(Train *v)
{
	...
	Train *tmp = v;
	Train *tmp2 = nullptr;
	for (Train *a = v->Next(); a != nullptr; a = a->Next()) {
		tmp->SetNext(tmp2);
		tmp2 = tmp;
		tmp = a;
	}
...
static Vehicle *TrainApproachingCrossingEnum(Vehicle *v, void *data)
{
	...
	if (!t->IsPrimaryVehicle()) return nullptr;
	...
	if (p2 != 0) {
		...
		if (flags & DC_EXEC) {
			...
			do {
				ToggleBit(t->flags, VRF_REVERSE_DIRECTION);
				t = t->Next();
			} while (t != nullptr && t->IsArticulatedPart());
...
void InheritWaitForCoupleOrders(Train *v, Train *u)
{
	...
	if (u->orders.list == nullptr && !OrderList::CanAllocateItem()) return;
...
bool TryPathReserve(Train *v, bool mark_as_stuck, bool first_tile_okay)
{
	 ...
	 * make matters worse. */
	if (other_train != nullptr && other_train->index != v->index && other_train->tile != v->tile) {
...
static Vehicle *CheckTrainAtSignal(Vehicle *v, void *data)
{
	...
	if (!t->IsPrimaryVehicle() || !(t->track & TRACK_BIT_MASK)) return nullptr;
